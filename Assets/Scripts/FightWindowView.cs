...
public class FightWindowView : MonoBehaviour
{
    ...
    [SerializeField] private TMP_Text _countDangerLevel;
    ...
    [SerializeField] private Button _addDangerLevel;
    [SerializeField] private Button _minusDangerLevel;
    ...
    private int _allCountDangerLevel;

    private int _stability = 2;
    ...
    private void Start()
    {
        ...
        _danger = new Danger(nameof(Danger));
        _danger.Attach(_enemy);
        ...
        _addDangerLevel.onClick.AddListener(() => ChangeDangerLevel(true));
        _minusDangerLevel.onClick.AddListener(() => ChangeDangerLevel(false));
    }
    ...
    private void OnDestroy()
    {
        ...
        _addDangerLevel.onClick.RemoveAllListeners();
        _minusDangerLevel.onClick.RemoveAllListeners(); 
        ...
        _danger.Detach(_enemy);
    }

    private void Fight()
    {
        if(_stability >= _allCountDangerLevel)
        {
            Debug.Log("No need for trouble, man!");
        }
        else
        {
    ...
    private void ChangeDangerLevel(bool isAddCount)
    {
        if (isAddCount)
            _allCountDangerLevel++;
        else
            _allCountDangerLevel--;

        ChangeDataWindow(_allCountDangerLevel, DataType.Danger);
    }
    ...
    private void ChangeDataWindow(int countChangeData, DataType dataType)
    {
        switch (dataType)
        {
            ...
            case DataType.Danger:
                _countDangerLevel.text = $"Danger Level: {countChangeData}";
                _danger.CountPower = countChangeData;
                break;
        }
